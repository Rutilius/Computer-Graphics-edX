...
{       
    if (enablelighting) {       
        ...
        vec4 myVert = modelview * myvertex;
        vec3 mypos = myVert.xyz / myVert.w ; // Dehomogenize current location 
        vec3 eyedirn = normalize(eyepos - mypos); 

        // Compute normal, needed for shading. 
        vec3 myNorm = mat3(transpose(inverse(modelview))) * mynormal;
        vec3 normal = normalize(myNorm); 
        ...
        for(int i = 0; i < numused; i++) {
            ...
            light += ComputeLight(direction, lightcolor[i], normal, halfv, diffuse, specular, shininess);
        }
        finalcolor = ambient + emission + light;
        //----------------------------------------------------------
